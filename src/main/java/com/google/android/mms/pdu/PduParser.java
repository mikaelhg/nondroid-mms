...
public class PduParser {
    ...
    protected static int skipWapValue(ByteArrayInputStream pduDataStream, int length) {
        assert(null != pduDataStream);
        if (length > 1000000) {
        	throw new RuntimeException("value length too large");
        }
        ...
        while(0 < lastLen) {
            ...
            switch (param) {
                ...
                case PduPart.P_CHARSET:
                    ...
                    if (((firstValue > TEXT_MIN) && (firstValue < TEXT_MAX)) ||
                        ...
                        try {
                        	...
                        	log.log(Level.WARNING, Arrays.toString(charsetStr), e);
                ...
                case PduPart.P_NAME:
                    ...
                    break;
                default:
                   	log.log(Level.WARNING, "Not supported Content-Type parameter");
                if (-1 == skipWapValue(pduDataStream, lastLen)) {
                	log.log(Level.WARNING, "Corrupt Content-Type");
                } else {
                    lastLen = 0;
                }
                break;
    ...
    protected static byte[] parseContentType(ByteArrayInputStream pduDataStream,
        ...
        if (cur < TEXT_MIN) {
            ...
            if (parameterLen < 0) {
            	log.log(Level.WARNING, "Corrupt MMS message");
                return (PduContentTypes.contentTypes[0]).getBytes(); //"*/*"
            }
    ...
    protected static boolean parsePartHeaders(ByteArrayInputStream pduDataStream,
            ...
            if (header > TEXT_MAX) {
                ...
                switch (header) {
                    ...
                    case PduPart.P_CONTENT_DISPOSITION:
                        ...
                        break;
                    default:
                       	log.log(Level.WARNING, "Not supported Part headers: " + header);
                    if (-1 == skipWapValue(pduDataStream, lastLen)) {
                       	log.log(Level.WARNING, "Corrupt Part headers");
                        return false;
                    }
                    lastLen = 0;
                    break;
        ...
        if (0 != lastLen) {
           	log.log(Level.SEVERE, "Corrupt Part headers");
            return false;
        }
